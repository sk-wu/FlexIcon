...
class ReferenceGenerator(nn.Module):
    ...
    def forward(self, c, x, color_theme=None, tps_reference=None):  # c is line art, x is reference image
        ...
        unaligned_style_image = x.detach().roll(1, 0).contiguous()  # 打印出风格图片
    ...
    # Palette-guided Inference
    def forward_inference_palette(self, c, x, color_theme=None):  # c is line art, x is reference image, color theme is reference features
        '''
        ReferenceBasedColorizer: contour (B 1 H W) + image (B 3 H W) -> output (B 3 H W)
        '''
        # Reconstruction Error
        ext1 = self.encode_content(c)  # content
        # style1 = self.encode_style(x)  # style
        rec11 = self.decode(ext1, color_theme)  # aligned
        return rec11
