...
if opt.fp16:
    ...
    if len(opt.gpu_ids) > 0:
        # optimizer_G, optimizer_D = model.module.optimizer_G, model.module.optimizer_D # 0321去掉
        optimizer_G = model.module.optimizer_G
        # added by wsk
        # optimizer_D_Refine = model.module.optimizer_D_Refine
        if not opt.no_domain_loss:
            optimizer_D_Domain = model.module.optimizer_D_Domain
    else:
        optimizer_G, optimizer_D = model.optimizer_G, model.optimizer_D
        # added by wsk
        # optimizer_D_Refine = model.optimizer_D_Refine
        if not opt.no_domain_loss:
...
for epoch in range(start_epoch, opt.niter + opt.niter_decay + 1):
    ...
    for i, data in enumerate(dataset, start=epoch_iter):
        ...
        losses, generated = model(Variable(data['label']), Variable(data['inst']),
                                  ...
                                  Variable(data['color_theme_reference']), Variable(data['tps_reference']), image_contrastive_list, infer=save_fake)
        ...
        loss_G = loss_dict['G_GAN'] + loss_dict.get('G_GAN_Feat', 0) + loss_dict.get('G_Palette', 0) + loss_dict.get('G_VGG_Refine', 0) + \
        ...
        if save_fake:
            visuals = OrderedDict([('input_label', util.tensor2label(data['label'][0], opt.label_nc)),
                                   # ('synthesized_using_palette_image', util.tensor2im(generated[0].data[0])),
                                   # ('synthesized_using_style_image', util.tensor2im(generated[1].data[0])),
                                   # ('contour_real_image', util.tensor2im(generated[2].data[0])),
                                   # ('contour_unaligned_image', util.tensor2im(generated[3].data[0])),
                                   # ('unaligned_style_image', util.tensor2im(generated[4].data[0])),
                                   ('real_image', util.tensor2im(data['image'][0])),
                                   ('theme_image', util.tensor2im(data['color_theme_reference'][0])),
                                   # ('tps_reference_image', util.tensor2im(data['tps_reference'][0]))
                                   ])
